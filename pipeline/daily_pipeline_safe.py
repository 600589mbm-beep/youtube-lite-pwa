#!/usr/bin/env python3
"""Cron-safe daily YouTube Shorts pipeline.
...
def main() -> int:
    ...
    render_background(background_source, background_path, TARGET_SECONDS, ffmpeg_binary)
...
def resolve_ffmpeg_binary(configured: str) -> str:
    ...
    candidates.extend(["/usr/bin/ffmpeg", "/usr/local/bin/ffmpeg", "/bin/ffmpeg"])
    ...
    raise SystemExit(
        "ffmpeg was not found. Set FFMPEG_BINARY in .env or install ffmpeg on the VPS."
    )
...
def render_background(source_path: Optional[Path], output_path: Path, duration_seconds: int, ffmpeg_binary: str) -> None:
...
def build_background_clip(source_path: Optional[Path], duration_seconds: int):
    ...
    if clip.duration < duration_seconds:
        ...
        clips = [clip.copy() for _ in range(repeats)]
...
def render_final_short(background_path: Path, audio_path: Path, subtitles_path: Path, output_path: Path, ffmpeg_binary: str) -> None:
    ...
    subtitle_filter = f"subtitles={escape_ffmpeg_path(subtitles_path)}:force_style='{subtitle_style()}'"
    command = [
        ffmpeg_binary,
        "-y",
        "-i",
...
def wait_for_publish(job_id: str, queue_endpoint: str, timeout_minutes: int, poll_seconds: int) -> bool:
    ...
    while time.monotonic() < deadline:
        data = retry_http_get(queue_endpoint)
    ...
    raise SystemExit("Timed out while waiting for the upload to finish.")
...
def create_thumbnail(source_path: Optional[Path], thumbnail_text: str, output_path: Path) -> None:
    ...
    image = image.resize((1280, 720))
    overlay = Image.new("RGBA", image.size, (0, 0, 0, 85))
    image = Image.alpha_composite(image, overlay)
    draw = ImageDraw.Draw(image)
    font = load_font(74)
    text = wrap_text_for_width(thumbnail_text.upper(), font, 1040)
...
def setup_background_clip(source_path: Optional[Path], duration_seconds: int) -> Any:
    if source_path is None:
        return ColorClip(size=TARGET_SIZE, color=(10, 14, 25), duration=duration_seconds)
...
def render_background_video(source_path: Optional[Path], output_path: Path, duration_seconds: int) -> None:
    clip = setup_background_clip(source_path, duration_seconds)
    try:
        clip.write_videofile(
            str(output_path),
            codec="libx264",
...
def retryable_publish_pipeline(topic: str, config: PipelineConfig, ffmpeg_binary: str, run_dir: Path, publish_delay_hours: int, dry_run: bool) -> Dict[str, Any]:
    ...
    background_source = choose_background_clip(config.background_dir)
    render_background_video(background_source, background_path, TARGET_SECONDS)
